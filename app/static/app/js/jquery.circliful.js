    ...
    $.fn.circliful = function (options, callback) {
        ...
        return this.each(function () {
            ...
            if (settings.icon !== 'none') {
                icon = '<text text-anchor="middle" x="' + iconX + '" y="' + iconY + '" class="icon" style="font-size: ' + settings.iconSize + 'px" fill="' + settings.iconColor + '">&#x' + settings.icon;
            } else {
                icon = '';
            }

            if (settings.percentages.length > 0) {
                for (var index = 0; index < settings.percentages.length; ++index) {
                    var mycolor, mypercent;
                    mycolor=settings.percentages[index].color;
                    mypercent=settings.percentages[index].percent;
                    icon+= '<text text-anchor="middle" x="' + iconX + '" y="' + (74+(index*30)) + '" class="icon" style="font-size: ' + (settings.iconSize-8) + 'px" fill="' + mycolor + '">' + settings.percentages[index].title + '</text>';
                    icon+= '<text text-anchor="middle" x="' + iconX + '" y="' + (90+(index*30)) + '" class="icon" style="font-size: ' + settings.iconSize + 'px" fill="' + mycolor + '">' + settings.percentages[index].percent + '%' + '</text>';
                    // lines += '<div id="legend-circle'+ (index+1) +'" style="font-size:' + settings.percentageTextSize + 'px;color:' + color + '">' + title + ': ' + percent + '%</div>';
                }
            }
            if (settings.halfCircle) {
            ...
            function drawCircles() {
                if (settings.percentages.length > 0) {
                    var index, calculateFillMulti, percent, color, title;
                    var percentages = settings.percentages;
                    var radius = 47;
                    var circleRadius = 360;
                    var rotate = -90;
